...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            " The directory must contain the membership file and also files 1.log, ... N.log for each process"
        )
        print(" The script prints last line CORRECT if all properties are satisfied")
        print(
            " The scripts prints last line INCORRECT with previous lines describing the issue if there were any"
        )
        sys.exit(0)
    ...
    dependencies = {
        i + 1: [int(t) for t in x.split()[1:]] for i, x in enumerate(dependencies)
    }
    ...
    for proc, deps in dependencies.items():
        print(
            "Process %d depends on {%s} (and itself)"
            % (proc, ", ".join([str(x) for x in deps]))
        )
    ...
    correct = processes  # [x for x in processes if x not in crashed]
    ...
    logs = {
        i: list(
            filter(
                lambda x: len(x) > 0,
                open(d + "/proc{:02d}.output".format(i), "r").read().split("\n"),
            )
        )
        for i in processes
    }
    ...
    delivered_by_from_set = {
        (i, j): set(arr) for ((i, j), arr) in delivered_by_from.items()
    }
    ...
    for p in correct:
        for msg in broadcast_by_set[p]:
            for p1 in correct:
                soft_assert(
                    msg in delivered_by_set[p1],
                    "BEB1 Violated. Correct %d broadcasted %s and correct %d did not receive it"
                    % (p, msg, p1),
                )
    ...
    for p in processes:
        for s in processes:
            ...
            soft_assert(
                len(delivered_by_p_f) == len(set(delivered_by_p_f)),
                "BEB2 Violated. Process %d delivered some messages from %d twice"
                % (p, s),
            )
    ...
    for p in processes:
        for p1 in processes:
            ...
            for msg in delivered:
                soft_assert(
                    msg in sent,
                    "BEB3 violated. Message %d was NOT send from %d and WAS delivered by %d"
                    % (msg, p, p1),
                )
    ...
    for msg in all_delivered:
        ...
        soft_assert(
            len(delivered_all) == 0 or len(notdelivered_correct) == 0,
            "URB4 Violated. Process %s delivered %d and correct %s did not deliver it"
            % (delivered_all, msg, notdelivered_correct),
        )
    ...
    for p in correct:
        ...
        for p1 in correct:
            ...
            for msg in delivered_by_p:
                soft_assert(
                    msg in delivered_by_p1
                )  # , "RB4 Violated. Correct %d delivered %d and correct %d did not deliver it" % (p, msg, p1))
    ...
    for p in processes:
        ...
        for event in events[p]:
            ...
            # property test
            soft_assert(
                vec_leq(msg_vc[msg], v_recv),
                "CRB5 violated: Process %d have delivered %s with vector clock W = %s having V_recv = %s"
                % (p, str(msg), str(msg_vc[msg]), str(v_recv)),
            )
